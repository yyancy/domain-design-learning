...
import com.huawei.charging.domain.charge.chargeplan.CompositeChargePlan;
...
public class Account {
    ...
    public static Account valueOf(long phoneNo, Money amount) {
        ...
        account.chargePlanList.add(new BasicChargePlan(new BasicChargeRule()));
        return account;
    }
    ...
    public List<ChargeRecord> charge(ChargeContext ctx) {
        CompositeChargePlan chargePlan = new CompositeChargePlan(chargePlanList);
        List<ChargeRecord> chargeRecords = chargePlan.doCharge(ctx);
